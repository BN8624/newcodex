...
- 단계, 증가 비용, 구매 가능 상태, 즉시 효과가 있는 성장 버튼 6개.
- 7초 재사용 대기시간의 월광 폭발 액티브 전투 버튼.
...
PASS: crit upgrade changes chance
...
인벤토리, 장비, 뽑기, 스킬 트리, 퀘스트, 수익화, 로그인, 클라우드 저장, 랭킹, 전체 음악 시스템, 앱 스토어 패키징, 최종 외주 아트 에셋은 v0.1에서 의도적으로 제외했다.

## 10. 다음 원클릭 스프린트 추천

Godot 4.7 Web 내보내기 템플릿을 설치하고 정식 Web 내보내기를 실행한 뒤, 저장 형식을 바꾸지 않고 보스별 특수 패턴과 두 번째 지역 데이터 조각을 추가한다.
